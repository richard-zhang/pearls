...
int main(int argc, char **argv) {
  ...
  fprintf(stderr, "FD_SETSIZE is %d\n", FD_SETSIZE);
  fprintf(stderr, "listenfd is %d\n", listenfd);
  while (1) {
    ...
    if (FD_ISSET(listenfd, &pool.ready_set)) {
      // fprintf(stderr, "listenfd %d is ready for listening\n", listenfd);
      connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);
      fprintf(stderr, "a new client %d is added\n", connfd);
...
void check_clients(pool *p) {
  ...
  for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
    ...
    if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) {
      fprintf(stderr, "**** connfd is ready%d\n", connfd);
      p->nready--;
      // connfd is ready
      // let's read from the connfd
      int num_bytes;
      num_bytes = Read(connfd, buf, sizeof(buf));
      if (num_bytes == 0)
      {
        // fprintf(stderr, "removed client listention on: %d", connfd);
        // close_and_remove(p, i);
        // continue;
      }
      memcpy(rb->requestbuf + rb->first_empty_byte, buf, num_bytes);
      rb->first_empty_byte += num_bytes;
      ...
      case incomplete:
        break;
      case root:
        fprintf(stderr, "sizeof ok is %lu and size of index html is %lu\n", sizeof(OK), sizeof(indexhtml));
        fprintf(stderr, "request buffer is %s", rb->requestbuf);
        writen(connfd, INDEX_HEADER, sizeof(INDEX_HEADER));
        writen(connfd, indexhtml, sizeof(indexhtml));
